<script>
	...
	import bill, { txntype, order, contact, payer, payoption, payoptions } from '$lib/stores/bill.js';
	...
	let title = 'Realty Tax Online Billing';
	let partner = {};
	...
	const confirmCheckout = (evt) => {
		payer.set(evt.detail.payer);
		payoptions.set(evt.detail.payoptions);
		mode = 'payoptions';
	};

	const selectPaymentPartner = (evt) => {
		payoption.set(evt.detail);
		mode = 'payment';
